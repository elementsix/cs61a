...
def find_closest(location, centroids):
    ...
    """
    "*** YOUR CODE HERE ***"
    return centroids[[distance(location, x) for x in centroids].index(min([distance(location, x) for x in centroids]))]
...
def find_centroid(restaurants):
    ...
    "*** YOUR CODE HERE ***"
    return [mean([restaurant_location(r)[0] for r in restaurants]), mean([restaurant_location(r)[1] for r in restaurants])]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    "*** YOUR CODE HERE ***"
    mean_x = mean(xs)
    s_xx = sum([pow(x_i - mean_x, 2) for x_i in xs])

    mean_y = mean(ys)
    s_yy = sum([pow(y_i - mean_y, 2) for y_i in ys])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    return max([find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns], key = lambda x: x[1])
...
def rate_all(user, restaurants, feature_functions):
    ...
    return {restaurant: user_rating(user, restaurant) if restaurant in user_reviewed_restaurants(user, restaurants)
            else predictor[0](restaurants[restaurant])
