...
def adv_parse(line):
    ...
    if token == 'give':
        ...
        object = tokens[:to]
        predicate = tokens[-1]
        return (token, ' '.join(object), predicate)
    elif token == 'ask':
        "*** YOUR CODE HERE ***"
        whom = tokens[0]
        what = ' '.join(tokens[2:])
        return (token, whom, what)
...
def adv_eval(exp):
    ...
    if is_person(exp):
        ...
        "*** YOUR CODE HERE ***"
        thing = adv_eval(exp[1])
        person = adv_eval(exp[2])
        return give(person, thing)

    elif exp[0] == 'ask':
        "*** YOUR CODE HERE ***"
        person = adv_eval(exp[1])
        message = adv_eval(exp[2])
        return ask(person, message)

    else:
        "*** YOUR CODE HERE ***"
        operator = adv_eval(exp[0])
        operand = adv_eval(exp[1])
        return adv_apply(operator, operand)
