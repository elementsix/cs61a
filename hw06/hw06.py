...
def deep_map(f, s):
    ...
    # violates DRY priniciple. but much clearer this way.

    elif isinstance(s.first, Link):
        mapped_link = Link(deep_map(f, s.first))
        mapped_link.rest = deep_map(f, s.rest)
        return mapped_link

    else:
        mapped_link = Link(f(s.first))
        mapped_link.rest = deep_map(f, s.rest)
        return mapped_link
...
def reverse(s):
    ...
    reversed_link = Link(s.first)
    x = s.rest
    
    while (x is not Link.empty):
        reversed_link = Link(x.first, reversed_link)
        x = x.rest
...
class Mobile:
    ...
    def weight(self):
        ...
        "*** YOUR CODE HERE ***"
        return sum([b.contents.weight for b in [self.left, self.right]])

    def is_balanced(self):
        """True if and only if the mobile is balanced."""
        "*** YOUR CODE HERE ***"

        balanced = self.left.torque == self.right.torque

        if balanced:
            if isinstance(self.left.contents, Mobile):
                balanced = balanced and self.left.contents.is_balanced()

            if isinstance(self.right.contents, Mobile):
                balanced = balanced and self.right.contents.is_balanced()
